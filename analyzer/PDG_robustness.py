...
def attack(classifier,x_test):
    attacker = ProjectedGradientDescent(classifier, norm = 1, eps=0.5, eps_step = 0.1, max_iter = 10, targeted = False, num_random_init = 0, batch_size = 1, random_eps = False, summary_writer = False, verbose = True )
...
def attack_robust(robust_classifier,x_train,y_train):
    attacker_robust= ProjectedGradientDescent(robust_classifier, norm = 1, eps=0.5, eps_step = 0.1, max_iter = 10, targeted = False, num_random_init = 0, batch_size = 1, random_eps = False, summary_writer = False, verbose = True )
    trainer = AdversarialTrainer(robust_classifier, attacker_robust, ratio=1.0)
    trainer.fit(x_train[:100], y_train, nb_epochs=15, batch_size=50)
...
def plot_graph(eps_range, nb_correct_original,nb_correct_robust,title):
    ...
    plt.savefig('FGSMevaluation{}.jpg'.format(title))
    return 'FGSMevaluation{}.jpg'.format(title)
...
<body>
    ...
    <br><br>
    <p>Attack : Projected Gradient Descent Attack </p>
...
def PGD_main(dataset):
    ...
    x_test_adv_pred, nb_correct_adv_pred = attacked_accuracy(classifier,x_test_adv,y_test)
