...
def attack(classifier,x_test):
    attacker = DeepFool(classifier, max_iter = 5, epsilon = 0.1, nb_grads = 9, batch_size = 1, verbose = True)
...
def attack_robust(robust_classifier,x_train,y_train):
    attacker_robust= DeepFool(robust_classifier, max_iter = 5, epsilon = 0.1, nb_grads = 9, batch_size = 1, verbose = True)
...
def plot_graph(eps_range, nb_correct_original,nb_correct_robust,title):
    ...
    plt.savefig('DeepFoolevaluation{}.jpg'.format(title))
    return 'DeepFoolevaluation{}.jpg'.format(title)
